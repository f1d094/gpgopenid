#!/usr/bin/env python
# a openpgp backed openid provider
# based onthe openid example script.
__copyright__ = 'Copyright GPL3+ 2013, Richard Ulrich'
	...
	def do_GET(self):
		try:
			...
			if 'true' != self.headers['X-Auth-OpenPGP'].lower().strip():
				self.send_error(401,"The http request was not signed by EnigForm")
...
"""%(discover.OPENID_IDP_2_0_TYPE, endpoint_url,))
	...
	def showPage(self, response_code, title, head_extras='', msg=None, err=None, form=None):
